...
export default function ManagerManageUsersPage() {
  ...
  // Fetch users (managers can only see users they created)
  useEffect(() => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Filter to only show users (managers can't see admins or other managers)
      const filteredUsers = fetchedUsers.filter(u => u.role === 'user')
      // Fixed: Convert AuthUser[] to UserData[] format
      const userData: UserData[] = filteredUsers.map(user => ({
        id: user.id,
        email: user.email,
