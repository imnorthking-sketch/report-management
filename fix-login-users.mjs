...
async function fixLoginUsers() {
  ...
  for (const user of defaultUsers) {
    try {
      ...
      if (existingAuthUser) {
        console.log(`Auth user ${user.email} exists`);
        ...
        try {
          ...
          if (updateError) {
            console.error(`Error updating auth user password for ${user.email}:`, updateError.message);
          } else {
            console.log(`Updated auth user password for ${user.email}`);
          }
        } catch (updateError) {
          console.error(`Error updating auth user for ${user.email}:`, updateError.message);
        }
      } else {
