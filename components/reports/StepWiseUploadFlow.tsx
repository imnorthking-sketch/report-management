...
export function StepWiseUploadFlow() {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (new Date(formData.reportDate) > new Date()) {
  ...
  const handleFilesAdded = (newFiles: File[]) => {
    // Check if report date is selected before uploading
    if (!formData.reportDate) {
      alert('Please select a report date before uploading files.')
      return
    }
 ...
 const uploadAndProcessFile = async (fileId: string) => {
  ...
  const handleSendForApproval = async () => {
    ...
    try {
      ...
      // Immediately redirect to payment page with report data
      const reportData = {
        id: result.reportId,
        title: formData.title || 'Expense Report',
        totalAmount,
      ...
      // Store payment data and redirect to appropriate payment page based on user role
      localStorage.setItem('pendingPayment', JSON.stringify(reportData))
      
      // Redirect based on user role
      if (user?.role === 'admin') {
        router.push('/admin/payment')
      } else if (user?.role === 'manager') {
        router.push('/manager/payment')
      } else {
